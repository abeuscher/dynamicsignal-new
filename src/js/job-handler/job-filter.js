...
function JobFilter(opts) {
  ...
  if (location.hash !== "") {
    ...
    if (initCategories[1] !== "undefined") {
      currentLoc = locations.filter(loc => { return loc.cat_name === decodeURIComponent(initCategories[1]) })[0]
    }
    if (initCategories[2] !== "undefined") {
      currentDept = departments.filter(loc => { return loc.cat_name === decodeURIComponent(initCategories[2]) })[0]
    }
  }
